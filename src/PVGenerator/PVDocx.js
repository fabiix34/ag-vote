...
import { documentService } from '../services/db.js';
...
// ─── Calcul votes (identique à PVDocument) ────────────────────────────────────
...
const CELL_NO_BORDERS = { top: NONE_BORDER, bottom: NONE_BORDER, left: NONE_BORDER, right: NONE_BORDER };
const TABLE_NO_BORDERS = { ...CELL_NO_BORDERS, insideH: NONE_BORDER, insideV: NONE_BORDER };

// ─── Composants paragraphes ───────────────────────────────────────────────────
...
function buildVoteSection(resolution, votes, coproprietaires) {
  ...
  if (!d.hasVotes && presents.length === 0) {
    items.push(new Paragraph({
      children: [new TextRun({ text: 'Pas de vote enregistré pour cette résolution.', italics: true, size: 18 })],
    }));
    return items;
  }
  ...
  items.push(new Paragraph({
    children: [new TextRun({ text: 'Mise aux voix, cette résolution a donné lieu au vote suivant :', size: 18 })],
    spacing: { after: 100 },
  }));

  // Tableau de votes (sans bordures)
  const voteRows = [
    { tant: formatFraction(d.tantPour, d.tantExprimes), label: 'Votes POUR', count: `${d.pourCopros.length} / ${d.countExprimes}` },
    { tant: formatFraction(d.tantContre, d.tantExprimes), label: 'Votes CONTRE', count: `${d.contreCopros.length} / ${d.countExprimes}` },
    { tant: formatFraction(d.tantAbst, d.tantPresents), label: 'Votes ABSTENTION', count: `${d.abstCopros.length} / ${d.countPresents}` },
  ];

  items.push(new Table({
    rows: voteRows.map(row => new TableRow({
      children: [
        new TableCell({
          children: [new Paragraph({ children: [new TextRun({ text: row.tant, bold: true, size: 18 })], alignment: AlignmentType.RIGHT })],
          width: { size: 32, type: WidthType.PERCENTAGE },
          borders: CELL_NO_BORDERS,
        }),
        new TableCell({
          children: [new Paragraph({ children: [new TextRun({ text: row.label, size: 18 })], alignment: AlignmentType.CENTER })],
          width: { size: 36, type: WidthType.PERCENTAGE },
          borders: CELL_NO_BORDERS,
        }),
        new TableCell({
          children: [new Paragraph({ children: [new TextRun({ text: row.count, bold: true, size: 18 })], alignment: AlignmentType.LEFT })],
          width: { size: 32, type: WidthType.PERCENTAGE },
          borders: CELL_NO_BORDERS,
        }),
      ],
  ...
  if (resolution.statut === 'en_cours' || resolution.statut === 'termine') {
    const voterGroups = [
      { copros: d.pourCopros, tant: d.tantPour, label: 'Se sont exprimés en faveur (dont non-votants)' },
    ...
    for (const { copros, tant, label } of voterGroups) {
      ...
      items.push(new Paragraph({
        children: [new TextRun({ text: copros.map(nomComplet).join(', '), italics: true, size: 17 })],
        indent: { left: 720 },
        spacing: { after: 120 },
      }));
    }
...
export async function generatePVDocx({ resolutions, votes, coproprietaires, documents }) {
  ...
  children.push(new Paragraph({
    children: [new TextRun({ text: 'ASSEMBLÉE GÉNÉRALE ORDINAIRE', bold: true, size: 24 })],
    alignment: AlignmentType.CENTER,
    spacing: { after: 120 },
  }));
  children.push(new Paragraph({
  ...
  if (presents.length === 0) {
    children.push(new Paragraph({
      children: [new TextRun({ text: 'Aucun copropriétaire enregistré comme présent.', size: 18 })],
    }));
  } else {
    children.push(new Paragraph({
      children: [new TextRun({
        text: presents.map(c => `${nomComplet(c)} (${formatTantiemes(c.tantiemes)} tants.)`).join(' — '),
        size: 18,
      })],
      spacing: { after: 120 },
    }));
  ...
  for (const [idx, r] of resolutions.entries()) {
    ...
    children.push(new Paragraph({
      children: [new TextRun({ text: `${idx + 1}.  ${r.titre.toUpperCase()}`, bold: true, underline: {}, size: 22 })],
    ...
    if (hasMontant) {
      children.push(new Paragraph({
        children: [
          ...
          new TextRun({ text: r.montant.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' }), bold: true, size: 18 }),
        ],
        spacing: { after: 120 },
      }));
    }
  ...
  // ── SIGNATURES ──
  children.push(spacer(1120));
  children.push(new Table({
    rows: [
      new TableRow({
        children: ['Le Président de séance', 'Le Scrutateur', 'Le Secrétaire de séance'].map(() =>
          new TableCell({
            children: [spacer(880)],
            borders: CELL_NO_BORDERS,
      ...
      new TableRow({
        children: ['Le Président de séance', 'Le Scrutateur', 'Le Secrétaire de séance'].map(role =>
          new TableCell({
            children: [new Paragraph({
              children: [new TextRun({ text: role, size: 18 })],
              alignment: AlignmentType.CENTER,
            })],
            borders: { top: { color: '000000', space: 1, style: BorderStyle.SINGLE, size: 6 }, ...CELL_NO_BORDERS },
            width: { size: 33, type: WidthType.PERCENTAGE },
          })
        ),
      }),
...
// ─── Téléchargement complet (fetch + génération + déclenchement) ──────────────

export async function downloadPVDocx({ resolutions, votes, coproprietaires }) {
  const documents = await fetchDocuments(resolutions.map(r => r.id));
  const base64 = await generatePVDocx({ resolutions, votes, coproprietaires, documents });

  const a = document.createElement('a');
  a.href = `data:application/vnd.openxmlformats-officedocument.wordprocessingml.document;base64,${base64}`;
  a.download = `PV_AG_${new Date().toISOString().slice(0, 10)}.docx`;
